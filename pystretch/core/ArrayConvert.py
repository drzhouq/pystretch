...
def ndarray_to_shmem(array):
    ...
    """
    arr = array.reshape((-1, ))
    datatype =_numpy_to_ctypes[array.dtype.type]
    data = RawArray(datatype, 
                                        arr.size)
    ctypes.memmove(data, array.ctypes.data_as(ctypes.POINTER(datatype)), arr.size*arr.strides[0])
    return data
