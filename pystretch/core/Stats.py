...
def get_array_percentile(array,  p):
    '''
    Calculates the statistics from a numpy array and returns a dictionary containing:
    mean, maximum, minimum, and potentially standard deviation.
    ...
    stats = {}
    if p > 0 and p < 100:
        stats['upperbound'] = float(numpy.percentile(array, 100-p))
        stats['lowerbound'] = float(numpy.percentile(array,p))
    del array
    gc.collect()
    return stats


def get_band_stats(band,args):
...
#    if args.clip > 0 and args.clip < 100: 
#        array = band.ReadAsArray(0, 0, band.XSize, band.YSize).astype(numpy.float32)
#        stats['upperbound'] = float(numpy.percentile(array, 100-args.clip))
#        stats['lowerbound'] = float(numpy.percentile(array,args.clip))
#        del array
#        gc.collect()
...
def scale(array, args):
    ...
    array += scalemin
    low_value_index = array < scalemin
    array[low_value_index] = scalemin
    high_value_index =  array > scalemax 
    array[high_value_index] =  scalemax
    return array
